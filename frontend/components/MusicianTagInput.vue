<template>
  <div class="musician-tag-input">
    <div class="tags">
      <div v-for="(entry, i) in modelValue" :key="i" class="tag">
        <span>{{ entry.musician_name }} — {{ entry.instrument }}</span>
    ...
    <div class="add-row">
      ...
      <input v-model="instrumentInput" placeholder="Instrument (e.g. drums)" />
...
const props = defineProps<{
  modelValue: { musician_name: string; instrument: string }[]
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', val: { musician_name: string; instrument: string }[]): void
}>()
...
async function onNameInput() {
  ...
  try {
    suggestions.value = await apiFetch<{ id: number; name: string }[]>(
      `/musicians?search=${encodeURIComponent(nameQuery.value)}`
    )
  } catch {
    suggestions.value = []
  }
...
<style scoped>
.musician-tag-input { display: flex; flex-direction: column; gap: 0.5rem; }
