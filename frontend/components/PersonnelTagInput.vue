<template>
  <div class="personnel-tag-input">
    <div class="tags">
      <div v-for="(entry, i) in modelValue" :key="i" class="tag">
        <span>{{ entry.person_name }} — {{ entry.role }}</span>
    ...
    <div class="add-row">
      ...
      <input v-model="roleInput" placeholder="Role (e.g. Producer)" />
...
const props = defineProps<{
  modelValue: { person_name: string; role: string }[]
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', val: { person_name: string; role: string }[]): void
}>()
...
async function onNameInput() {
  ...
  try {
    suggestions.value = await apiFetch<{ id: number; name: string }[]>(
      `/persons?q=${encodeURIComponent(nameQuery.value)}`
    )
  } catch {
    suggestions.value = []
  }
...
<style scoped>
.personnel-tag-input { display: flex; flex-direction: column; gap: 0.5rem; }
