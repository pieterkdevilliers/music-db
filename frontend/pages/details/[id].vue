    ...
    <div v-if="detail">
      ...
      <div class="section">
        ...
        <table v-if="albums.length" class="disc-table">
          <thead>
            <tr><th>Album</th><th>Artist</th><th>Year</th><th>Type(s)</th></tr>
          </thead>
          <tbody>
        ...
        <p v-else class="empty">No albums found for this detail.</p>
      </div>
    </div>
...
const albums = ref<{ album: AlbumSummary; types: string[] }[]>([])

onMounted(async () => {
  const data = await apiFetch<{ detail: DetailItem; albums: { album: AlbumSummary; types: string[] }[] }>(
    `/details/${id}`
  )
